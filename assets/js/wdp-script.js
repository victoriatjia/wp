jQuery(document).ready(function ($) {
  ...
  $(document).on('submit', '.wdp-container-form form', function (event) {
    ...
    //Si el comentario tiene menos de 2 caracteres no se enviarÃ¡
    if ($content.length < 2) {
      ...
      //Si el formulario estÃ¡ validado
      if (form_ok === true) {
  ...
  function getComments_WDP(post_id, num_comments, num_get_comments, order_comments) {
    ...
    if (num_comments > 0) {
      jQuery.ajax({
        type: "POST",
        dataType: "html",// tipo de informaciÃ³n que se espera de respuesta
        url: WDP.ajaxurl,
        data: {
          action: 'get_comments',
          post_id: post_id,
        ...
        success: function (data) {
          ...
          //Insertamos PaginaciÃ³n de Comentarios
  ...
  //MOSTRAR/OCULTAR MÃS COMENTARIOS
  ...
  //   $modalHtml = '<div id="wdp-modal-wrap"><span id="wdp-modal-close"></span><div id="wdp-modal-header"><h3 id="wdp-modal-title">TÃ­tulo</h3></div><div id="wdp-modal-content"><p>Hola</p></div><div id="wdp-modal-footer"><a id="wdp-modal-ok-' + post_id + '" class="wdp-modal-ok wdp-modal-btn" href="#">' + WDP.accept + '</a><a class="wdp-modal-cancel wdp-modal-btn" href="#">' + WDP.cancel + '</a></div></div>';
  ...
  function jPages_WDP(post_id, $numPerPage, $destroy) {
    //Si existe el plugin jPages y estÃ¡ activado
    if (typeof jQuery.fn.jPages == 'function' && WDP.jpages == 'true') {
      ...
      if (num_comments > $numPerPage) {
        ...
        jQuery($holder).show().jPages({
          containerID: $idList,
          previous: "â† " + WDP_WP.textNavPrev,
          next: WDP_WP.textNavNext + " â†’",
